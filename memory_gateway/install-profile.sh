...
APP_HOME_ESCAPED="${APP_HOME//\"/\\\"}"
PROFILE_ESCAPED="${PROFILE//\"/\\\"}"
MEMORY_LOG_PATH_ESCAPED="${APP_HOME_ESCAPED}/memory/events.jsonl"
VAULT_PATH_ESCAPED="${APP_HOME_ESCAPED}/vault"
POSTGRES_DSN_ESCAPED="${POSTGRES_DSN//\"/\\\"}"
NEO4J_URI_ESCAPED="${NEO4J_URI//\"/\\\"}"
NEO4J_USER_ESCAPED="${NEO4J_USER//\"/\\\"}"
NEO4J_PASSWORD_ESCAPED="${NEO4J_PASSWORD//\"/\\\"}"
HELPER_MODEL_ESCAPED="${HELPER_MODEL//\"/\\\"}"
HELPER_BASE_URL_ESCAPED="${HELPER_BASE_URL//\"/\\\"}"
HELPER_TIMEOUT_ESCAPED="${HELPER_TIMEOUT_SEC//\"/\\\"}"
cat > "$CONFIG_PATH" <<EOF
AI_MEMORY_BRAIN_HOME="$APP_HOME_ESCAPED"
AI_MEMORY_INSTALL_PROFILE="$PROFILE_ESCAPED"
MEMORY_SERVER_HOST="127.0.0.1"
MEMORY_SERVER_PORT="8765"
MEMORY_GROUP_ID="personal-brain"
MEMORY_LOG_PATH="$MEMORY_LOG_PATH_ESCAPED"
VAULT_PATH="$VAULT_PATH_ESCAPED"
POSTGRES_DSN="$POSTGRES_DSN_ESCAPED"
NEO4J_URI="$NEO4J_URI_ESCAPED"
NEO4J_USER="$NEO4J_USER_ESCAPED"
NEO4J_PASSWORD="$NEO4J_PASSWORD_ESCAPED"
MEMORY_HELPER_ENABLED="$MEMORY_HELPER_ENABLED"
MEMORY_HELPER_MODEL="$HELPER_MODEL_ESCAPED"
MEMORY_HELPER_BASE_URL="$HELPER_BASE_URL_ESCAPED"
MEMORY_HELPER_TIMEOUT_SEC="$HELPER_TIMEOUT_ESCAPED"
EOF
